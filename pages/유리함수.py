...
CONCEPT_EXPLANATIONS = {
    "유리함수": (
        ...
        "유리함수의 그래프는 반드시 수직 및 수평 점근선(Asymptote)을 가지는 것이 핵심 특징입니다."
    ),
    "정의역": (
...
def get_concept_summary(concept_name: str, length: int) -> str:
    ...
    실제 LLM 호출 대신 미리 정의된 텍스트를 사용합니다.
    """
    ...
    # 선택된 줄들을 다시 줄바꿈 문자('\\n' 대신 실제 Markdown 줄바꿈을 위해 띄어쓰기 2개 후 \n)로 연결합니다.
...
# 참고: Streamlit에서 수식(Latex)을 줄바꿈 없이 깔끔하게 표시하기 위해 st.latex를 사용했습니다.
